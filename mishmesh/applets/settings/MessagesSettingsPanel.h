...
// (1/2) picked via a stepper modal, and a "Quick replies" row that drills into
// the canned-message manager. Source of truth: MessagesService + quickReplyStore().
...
private:
  struct Model : ListModel {
    ...
    enum Row : int { AutoRetry, AutoResetPath, DirectAcks, QuickReplies, ROW_COUNT };
    ...
    const char* value(int i) const override;   // DM-acks "1"/"2"; reply count on QuickReplies
  } _model;
