...
public:
  ...
  ArduboyGate() : _last(0), _started(false), _ab(false) {}

  // True at most once per FRAME_MS; resets the timer to `now` (no multi-step catch-up).
  bool stepDue(uint32_t now) {
    if (!_started) { _started = true; _last = now; return true; }
    if ((uint32_t)(now - _last) >= FRAME_MS) { _last = now; return true; }
    return false;
  }
